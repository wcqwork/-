    ...
    <div>
        ...
        <div class="flex-j-s-b" style="margin-bottom: 15px;">
            <el-row class="flex-a-center">
                <strong class="company-title">文章库</strong>
            </el-row>
            <el-row>
                <el-button disabled :icon="Plus">抓取文章</el-button>
            </el-row>
        </div>
        <!-- 提示条 -->
        <div class="tips" v-if="!tipFlag || tipFlag == 'true'">
            <svg-icon name="提醒" className="提醒" size="12" style="margin-right:12px;"></svg-icon>
            现阶段不支持系统抓取文章，请联系技术帮忙抓取！
            <!-- <span @click="closeTip()">
        ...
        <div class="intelligenceTable">
            ...
            <div class="flex-j-s-b t-set">
                <el-row class="r-serch">
                    <div class="searchval">
                        ...
                        <el-input v-model="searchContent" placeholder="请输入关键词，检索文章标题、内容" clearable />
                    </div>
                    <!-- <div class="searchval">
                        <i class="iconfont icon-sousuo"></i>
            ...
            <Tables :configTable="configTable" @sortChange="sortChange" headerCellClassName="HeadTh" :getList="getList"
                height="calc(100vh - 318px)" rowClassName="indexTable">
                <!-- 描述文本 -->
                <template v-slot:articleTitle="slotData">
                    <div class="overflow-text4">
                        <el-tooltip class="box-item" effect="dark" placement="top">
                            <template #content>
                                <div style="max-width:450px">{{ slotData.data.articleTitle }}</div>
                            </template>
                            <span>{{ slotData.data.articleTitle }}</span>
                        </el-tooltip>
                    </div>
                </template>
                <!-- 前/后缀 -->
                <template v-slot:articleContent="slotData">
                    <div class="overflow-text5">
                        <el-tooltip class="box-item" effect="dark" placement="top">
                            <template #content>
                                <div style="max-width:450px"> {{ slotData.data.articleContent }}</div>
                            </template>
                            <span> {{ slotData.data.articleContent }}</span>
                        </el-tooltip>
                    </div>
                </template>
                <!-- <template v-slot:relationKeyword="slotData">
                    <div style="display: flex; align-items: center;">
                        {{ slotData.data.relationKeyword }}
                    </div>
                </template> -->
        ...
        <el-dialog v-model="delDialogVisible" title="删除提醒" width="460px">
            <span style="line-height: 1.8;">您确定要删除当前文章吗？删除后下次再生成页面时将不会调用该文章，但是不影响已生成的页面。</span>
            <template #footer>
        ...
        <!-- 编辑文章 -->
        <el-dialog v-model="editArticleVisible" title="编辑文章" width="620px">
            <div class="addPageContent">
                <div class="addpage-item">
                    ...
                    <div class="addpage-cont">
                        <el-input v-model="articleTitleArea" maxlength="200" placeholder="请输入文章标题" show-word-limit
                            type="textarea" />
                    </div>
                </div>
                <div class="addpage-item">
                    ...
                    <div class="addpage-cont addpage-cont-c">
                        <el-input v-model="articleContentArea" maxlength="2000" placeholder="请输入文章内容" show-word-limit
                            type="textarea" />
                    </div>
                </div>
            </div>
            <template #footer>
                <span class="dialog-footer">
                    <el-button @click="handlerEditArticleCalc">取消</el-button>
                    <el-button type="primary" @click="handlerEditArticleOk(handlerUpdate)">确定</el-button>
                </span>
            </template>
        </el-dialog>
    </div>
...
import useEditArticle from "./child/useEditArticle"
...
// api
import { getPageDescList, changeStatus, setDelete, setAdd, setUpdate } from "@/api/resource/articleLibApi"
...
let { articleId,
    ...
    handlerEditDialog } = useEditArticle();
...
configTable = reactive({
    tHead: [
        ...
        },
        {
            ...
            width: 180,
            slot: true,
            slotName: "articleContent",
        },
        // { prop: "relationKeyword", label: "关联关键词", width: 120, slot: true, slotName: "relationKeyword" },
...
let getList = () => {
    let params = {
        ...
        relationKeyword: searchkeyWords.value,
...
let handlerEdit = (obj) => {
    articleId.value = obj.encodeId;
    handlerEditDialog(obj);
}

// 编辑更新
let handlerUpdate = () => {
    let params = {
        encodeId: articleId.value,
        articleTitle: articleTitleArea.value,
        articleContent: articleContentArea.value,
    }
