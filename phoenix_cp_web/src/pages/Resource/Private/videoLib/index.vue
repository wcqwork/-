    ...
    <div>
        ...
        <div class="intelligenceTable">
            ...
            <div class="flex-j-s-b t-set">
                <el-row class="r-serch">
                    <div class="searchval">
                        ...
                        <el-input v-model="searchContent" placeholder="请输入视频标题" clearable />
                    </div>
                    <div class="searchval">
                        <i class="iconfont icon-sousuo"></i>
            ...
            <Tables :configTable="configTable" @sortChange="sortChange" headerCellClassName="HeadTh" :getList="getList"
                ...
                <!-- 描述文本 -->
                <template v-slot:videoTitle="slotData">
                    <div style="display: flex; align-items: center;">
                        <el-tooltip class="box-item" effect="dark" placement="top">
                            <template #content>
                                <div style="max-width:450px">{{ slotData.data.videoTitle }}</div>
                            </template>
                            <span>{{ slotData.data.videoTitle }}</span>
                        </el-tooltip>
                    </div>
                </template>
                <!-- 前/后缀 -->
                <template v-slot:videoUrl="slotData">
                    <div style="display: flex; align-items: center;">
                        {{ slotData.data.videoUrl }}
                    </div>
                </template>
        ...
        <el-dialog v-model="delDialogVisible" title="删除提醒" width="460px">
            <span style="line-height: 1.8;">您确定要删除当前视频吗？删除后下次再生成页面时将不会调用该视频，但是不影响已生成的页面。</span>
            <template #footer>
...
// api
import { getPageDescList, changeStatus, setDelete, setAdd } from "@/api/resource/videoLibApi"
...
let getList = () => {
    let params = {
        videoTitle: searchContent.value,
